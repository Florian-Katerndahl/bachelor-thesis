...
streets <- st_read("D:\\Florian_Katerndahl\\bachelor\\data\\experiment_8_street_network.shp", quiet = TRUE)
...
tree_coords <- st_coordinates(tree_locations) %>% 
	...
	mutate(species = "",
		   ...
		   scale = rnorm(n(), 0.019, 0.001)) %>% # laubwerk scale: rnorm(n(), 0.019, 0.001); LumenRT(?) scale: rnorm(n(), 0.19, 0.01)
	relocate(species, .before = X)


# WARNING: For testing purposes, I only export `n_trees` trees
write.table(tree_coords, "D:\\Florian_Katerndahl\\bachelor\\data\\experiment_8_tree_attributes.txt",
